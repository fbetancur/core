...
function init(survey) {
    console.log('🔄 form-cache.init() called with:', survey);
    
    return store
        .init()
        .then(() => {
            // MODIFICACIÓN: Forzar actualización si queremos usar archivos locales
            console.log('🔄 Forzando uso de archivos locales...');
            return set(survey); // Siempre usar set() para forzar archivos locales
        })
 ...
 */
function set(survey) {
    // MODIFICACIÓN: Activar modo de archivos locales
    const surveyWithLocalFiles = { ...survey, useLocalFiles: true };
    
    console.log('🔄 form-cache.set() - Activando archivos locales');
    console.log('Survey original:', survey);
    console.log('Survey con archivos locales:', surveyWithLocalFiles);
    
    return connection
        .getFormParts(surveyWithLocalFiles)
    ...
    return connection
        ...
        .then((version) => {
            if (hash === version) {
                ...
                return connection
                    .getFormParts({ ...survey, useLocalFiles: true })
                    .then((formParts) => {
