...
async function getFormParts(props) {
    ...
    console.log('getFormParts called with props:', props);

    try {
        // MODIFICACIÓN: Usar archivos locales en lugar de servidor
        if (props.useLocalFiles) {
            console.log('🔄 Usando archivos locales...');
            // Cargar form.txt y model.txt desde archivos locales
            const formResponse = await fetch('./form.txt');
            const modelResponse = await fetch('./model.txt');
            
            console.log('Form response status:', formResponse.status);
            console.log('Model response status:', modelResponse.status);
            
            if (!formResponse.ok || !modelResponse.ok) {
                throw new Error('No se pudieron cargar los archivos locales');
            }
            
            let form = await formResponse.text();
            let model = await modelResponse.text();
            
            // Limpiar formato JSON y caracteres de escape
            ...
            // Limpiar escapes y caracteres de control
            form = form.replace(/\\"/g, '"')
                      .replace(/\\'/g, "'")
                      .replace(/\\r\\n/g, '\n')
                      .replace(/\\n/g, '\n')
            ...
            console.log('✅ Archivos locales cargados exitosamente');
            console.log('Form size:', form.length);
            console.log('Model size:', model.length);
            
            transformed = {
                form: form,
                model: model,
                hash: 'local-hash-456-updated', // Hash diferente para forzar actualización
                languageMap: {},
                theme: 'kobo' // Tema por defecto
            };
        } else if (props.xformUrl) {
